...
export default function IPS() {
  ...
  useEffect(() => {
    api.getCedulas()
      .then(res => { setCedulas(res.data || []); if (res.data?.length > 0) { const cap = res.data.find(c => (c.nombre || '').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().includes('capsula')); setCedulaId((cap || res.data[0]).id) } })
